...
def process_images(image_list, im_path):

    #im_path ='/home/pi/github/flaskviewer/static/Images'  
    ...
    for f, im in good_images:
        if len(im.shape) > 2:
            ...
            clip = grey[min_y:max_y, min_x: max_x]
        else:
    ...
    if len(best_set) > 2:
        for f, img in image_list:
            ...
            #    plt.show()
                cv2.imwrite(os.path.join(im_path, "best1.jpg"), img)
                return_files.append(f)
            if best_set[1][0] == f:
            #    plt.imshow(img, cmap = 'gray')
            #    plt.show()
                cv2.imwrite(os.path.join(im_path, "best2.jpg"), img)
                return_files.append(f)
            if best_set[2][0] == f:
            #    plt.imshow(img, cmap = 'gray')
            #    plt.show()
                cv2.imwrite(os.path.join(im_path, "best3.jpg"), img)
                return_files.append(f)
    return return_files
